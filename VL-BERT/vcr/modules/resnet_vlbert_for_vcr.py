...
class ResNetVLBERT(Module):
    def __init__(self, config):
        ...
        if not config.NETWORK.BLIND:
            ...
            if config.NETWORK.VLBERT.object_word_embed_mode == 1:
                self.object_linguistic_embeddings = nn.Embedding(1601, config.NETWORK.VLBERT.hidden_size)
            ...
            if self.enable_cnn_reg_loss and self.cnn_loss_top:
                self.cnn_loss_reg = nn.Sequential(
                    ...
                    nn.Linear(config.NETWORK.VLBERT.hidden_size, 1601)
                )
    ...
    def _collect_obj_reps(self, span_tags, object_reps):
        ...
        span_tags_fixed = torch.clamp(span_tags, min=0).cpu()  # In case there were masked values here
        # row_id = torch.zeros(span_tags_fixed.shape).long()
        # row_id_broadcaster = torch.arange(0, row_id.shape[0], step=1)[:, None]
        row_id = span_tags_fixed.new_zeros(span_tags_fixed.shape)
        row_id_broadcaster = torch.arange(0, row_id.shape[0], step=1, device=row_id.device)[:, None]
        # print('DEVICE: {} {} {} {}\n'.format(span_tags_fixed.device, span_tags.device, object_reps.device, row_id.device))
    ...
    def train_forward(self,
        ...
        #print(objects.shape, (obj_reps['obj_reps'].unsqueeze(1).repeat(1, num_choices, 1, 1)).shape, object_linguistic_embeddings.shape)
