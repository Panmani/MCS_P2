...
class VCTemporalCaptionsDataset(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, seq_len=64,
                 ...
                 add_image_as_a_box=False, feature_path='data/vcr/VCR_resnet101_faster_rcnn_genome.lmdb',
                 aspect_grouping=False, **kwargs):
        """
        VisualComet Temporal Captions Dataset
        ...
        :param kwargs:
        """
        super(VCTemporalCaptionsDataset, self).__init__()
        ...
        print('Caption sample:', np.random.choice(self.annot_data[0]['captions'], size=1)[0])
    ...
    def __getitem__(self, index):
        ...
        image_id = self._converId(annot_i['img_id'])
        ...
        if _p < 0.5 or (not self.with_rel_task):
            ...
            caption = np.random.choice(annot_i['captions'], size=1)[0]
        else:
            relationship_label = 0
            rand_index = random.randrange(0, len(self.annot_data))
            while rand_index == index:
                rand_index = random.randrange(0, len(self.annot_data))
            caption = np.random.choice(self.annot_data[rand_index]['captions'], size=1)[0]
