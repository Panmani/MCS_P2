...
from dataloaders.coco_generation import COCOGenDataset
...
def load_and_cache_coco_examples(args, config, tokenizer):
    # ========================== ianma_begin ==========================
    # dataset = VCGGenDataset(
    dataset = COCOGenDataset(
    # ========================== ianma_end ==========================
        ...
        num_train_part=args.num_train_part,
        # ========================== ianma_end ==========================
...
def beam_search_sequence(model, length, context, img_feats, boxes, boxes_mask, subject_ids,
                    end_token_id, pad_token_id, num_beams=1, temperature=1, device=None):
...
def sample_sequence(model, length, context, end_token, pad_token, img_feats, boxes, boxes_mask, subject_ids,
                    do_sample=True, num_samples=1, temperature=1, top_k=0, top_p=0.0, device=None):
    if not do_sample:
        ...
        return beam_search_sequence(model, length, context, img_feats, boxes, boxes_mask, subject_ids, end_token, pad_token,
                                    num_beams=num_samples, temperature=1, device=device)
    if device is None:
        device = torch.device("cpu")
...
def main():
    ...
    # ========================== ianma_begin ==========================
    parser.add_argument('--num_train_part', type=int, default=1,
                       help="Number of parts of the training set")
    ...
    model.transformer.use_subject_ids = False
    ...
    def _prompt_to_gen(context_orig, img_feats, boxes, boxes_mask, subject_ids):
    # ========================== ianma_end ==========================
    ...
    # Get Dataset Loader
    dataset = load_and_cache_coco_examples(args, config, tokenizer)
    ...
    output_keys = ['img_fn', 'metadata_fn', 'split', 'event', 'inference_relation', 'event_idx']
    # ========================== ianma_end ==========================
    results = []
    idx = 0
    context_inputs = set()
    for input_record, (
        ...
        if args.include_image:
            ...
            inputs, img_feats, boxes, boxes_mask, subject_ids = \
                [d.to(args.device) for d in data_input]
        else:
            ...
            img_feats, boxes, boxes_mask, subject_ids = [None] * 4
        ...
        generations = _prompt_to_gen(inputs, img_feats, boxes, boxes_mask, subject_ids)
