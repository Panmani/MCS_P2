...
from dataloaders.vcg_detected_boxes import VCGDataset
    ...
    for epoch in train_iterator:
        ...
        for step, data_input in enumerate(epoch_iterator):
            ...
            if args.include_image:
                ...
                inputs, labels, img_feats, boxes, boxes_mask = [
                    d.to(args.device) for d in data_input]
            else:
                ...
                img_feats, labels, boxes, boxes_mask, objects = [None] * 5
            ...
            outputs = model(inputs, labels=labels, img_feats=img_feats, boxes=boxes, boxes_mask=boxes_mask)
...
def evaluate(args, model, tokenizer,
    ...
    for data_input in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if args.include_image:
            ...
            inputs, labels, img_feats, boxes, boxes_mask = [d.to(args.device) for d in data_input]
        else:
            inputs, labels = [d.to(args.device) for d in data_input]
            # img_feats, boxes, boxes_mask, objects, segments, person_ids, subject_ids = [None] * 7
            img_feats, labels, boxes, boxes_mask, objects = [None] * 5
