...
from config import VCR_IMAGES_DIR, VCR_FEATURES_DIR, COCO_IMAGES_DIR, COCO_FEATURES_DIR
...
class COCOGenDataset:
    def __init__(self,
        ...
        self.train_parts = []
        if num_train_part == 1:
            self.train_parts = ["train"]
        elif num_train_part > 1:
            for part_id in range(num_train_part):
                train_part = "train_p{}_".format(part_id)
        ...
        splits = ['val'] + self.train_parts

        # for split in ['train', 'val', 'test']:
        for split in splits:
            split_filename = '{}2017_annots.json'.format(split)
        ...
        for split in splits:
            if len(split) >= 5:
                ...
                img_split = split
            self.vcg_dataset[split] = COCOGenLoader(examples[split], records[split], img_split, tokenizer,
...
class COCOGenLoader:
    ...
    def get_dets_to_use(self, item):
        ...
        with open(os.path.join(COCO_IMAGES_DIR, item['metadata_fn']), 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(COCO_IMAGES_DIR, record['metadata_fn']), 'r') as f:
            metadata = json.load(f)
        ...
        with open(os.path.join(COCO_FEATURES_DIR, "{}2017_feat".format(self.split), id+'.pkl'),'rb') as p:
        ...
        return event_inference_example, features, boxes, boxes_mask, subject_ids
        # ========================== ianma_end ==========================
