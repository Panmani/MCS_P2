...
from config import VCR_IMAGES_DIR, VCR_FEATURES_DIR, VCR_FEATURES_DIR_DET_BOX
        ...
        if os.path.exists(cached_features_files) and not overwrite_cache:
            ...
            for s, split in enumerate(splits):
                ...
                for record in tqdm(records[split], "Encoding Data"):
                    ...
                    if split == 'train':
                        e_l, r_l = [len(vt) for vt in tokens[1:]]
                        max_event = max(max_event, e_l)
                        # max_place = max(max_place, p_l)
                        max_inference = max(max_inference, r_l)
                    # ========================== ianma_end ==========================
                ...
                if split == 'train':
                    ...
                    print('max_image, max_event, max_inference:', max_image, max_event, max_inference)
                ...
                for tokens in tqdm(token_list):
                    ...
                    padded_tokens = _combine_and_pad_tokens(tokenizer, tokens, max_image, max_event, max_inference, max_seq_len)
...
class VCGLoader:
    ...
    def get_dets_to_use(self, item):
        ...
        with open(os.path.join(VCR_FEATURES_DIR_DET_BOX, os.path.basename(item['metadata_fn'])), 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(VCR_FEATURES_DIR_DET_BOX, os.path.basename(record['metadata_fn'])), 'r') as f:
            metadata = json.load(f)
        ...
        with open(os.path.join(VCR_FEATURES_DIR_DET_BOX, id+'.pkl'),'rb') as p:
            features_dict = pickle.load(p)
        features = features_dict['object_features'][dets2use]
        boxes = np.array(metadata['boxes'])[dets2use, :]
        ...
        subject_ids = [int(dets2use[i] in subjects) for i in range(len(dets2use))]
        ...
        # print(event_inference_example.shape, features.shape, boxes.shape, boxes_mask.shape, subject_ids.shape)
        # ========================== ianma_begin ==========================
        # return event_inference_example, features, boxes, boxes_mask, objects, segments, person_ids, subject_ids
        return event_inference_example, labels, features, boxes, boxes_mask
        # ========================== ianma_end ==========================
